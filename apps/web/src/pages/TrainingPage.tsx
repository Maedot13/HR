...
function ProgramForm({ onClose }: { onClose: () => void }) {
  ...
  const save = useMutation({
    mutationFn: () => api.post("/training/programs", { ...form, competencies: form.competencies.split(",").map(s => s.trim()).filter(Boolean) }),
  ...
  return (
    <Stack spacing={2} sx={{ mt: 1 }}>
      ...
      <TextField label="Competencies (comma-separated)" value={form.competencies} onChange={e => setForm(p => ({ ...p, competencies: e.target.value }))} />
      <Button variant="contained" onClick={() => save.mutate()} disabled={save.isPending}>Create Program</Button>
    </Stack>
  );
}
...
function AssignForm({ employeeId, onClose }: { employeeId: string; onClose: () => void }) {
  ...
  const { data: programsData } = useQuery<{ data: TrainingProgram[] }>({ queryKey: ["training-programs"], queryFn: () => api.get("/training/programs").then(r => r.data) });
  const programs: TrainingProgram[] = programsData?.data ?? [];

  const save = useMutation({
    mutationFn: () => api.post(`/employees/${employeeId}/training`, { ...form, expectedCompletion: new Date(form.expectedCompletion).toISOString() }),
  ...
  return (
    <Stack spacing={2} sx={{ mt: 1 }}>
      ...
      <FormControl required>
        ...
        <Select value={form.trainingProgramId} label="Program" onChange={e => setForm(p => ({ ...p, trainingProgramId: e.target.value }))}>
          {programs.map(p => <MenuItem key={p.id} value={p.id}>{p.title}</MenuItem>)}
        </Select>
      </FormControl>
      <TextField label="Expected Completion" type="date" value={form.expectedCompletion} onChange={e => setForm(p => ({ ...p, expectedCompletion: e.target.value }))} InputLabelProps={{ shrink: true }} required />
      <Button variant="contained" onClick={() => save.mutate()} disabled={save.isPending}>Assign</Button>
    </Stack>
  );
}
...
export default function TrainingPage() {
  ...
  const { data: programsData, isLoading: programsLoading } = useQuery<{ data: TrainingProgram[] }>({ queryKey: ["training-programs"], queryFn: () => api.get("/training/programs").then(r => r.data) });
  const { data: assignmentsData, isLoading: assignmentsLoading } = useQuery<{ data: TrainingAssignment[] }>({ queryKey: ["training-assignments", empId], queryFn: () => api.get(`/employees/${empId}/training`).then(r => r.data), enabled: !!empId });
  const { data: gapData } = useQuery<{ data: { required: string[]; completed: string[]; gaps: string[] } }>({ queryKey: ["skill-gap", empId], queryFn: () => api.get(`/employees/${empId}/skill-gap`).then(r => r.data), enabled: !!empId });

  const markComplete = useMutation({
    mutationFn: (id: string) => api.put(`/training/assignments/${id}/complete`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["training-assignments", empId] }),
  });
    ...
    <Box>
      ...
      <Paper>
        ...
        <Box sx={{ p: 2 }}>
          {tab === 0 && (
            <Box>
              {canManage && <Button variant="contained" onClick={() => setCreateOpen(true)} sx={{ mb: 2 }}>New Program</Button>}
              {programsLoading ? <Typography>Loading...</Typography> : (
                <Table size="small">
                  <TableHead><TableRow><TableCell>Title</TableCell><TableCell>Description</TableCell><TableCell>Competencies</TableCell></TableRow></TableHead>
          ...
          {tab === 1 && (
            <Box>
              <Stack direction="row" spacing={2} sx={{ mb: 2 }} alignItems="center">
                ...
                {canManage && <Button variant="contained" onClick={() => setAssignOpen(true)}>Assign Training</Button>}
              </Stack>
                ...
                <Table size="small">
                  ...
                  <TableBody>{assignments.map(a => (
                    <TableRow key={a.id}>
                      ...
                      <TableCell><Chip label={a.status} color={a.status === "COMPLETED" ? "success" : "warning"} size="small" /></TableCell>
                      {canManage && <TableCell>{a.status !== "COMPLETED" && <Button size="small" color="success" onClick={() => markComplete.mutate(a.id)}>Mark Complete</Button>}</TableCell>}
                    </TableRow>
                  ))}</TableBody>
                </Table>
              )}
          ...
          {tab === 2 && (
            <Box>
              ...
              {gap && (
                <Stack spacing={1}>
                  <Typography variant="subtitle2">Completed Competencies: {gap.completed.join(", ") || "None"}</Typography>
                  <Typography variant="subtitle2">Gaps: {gap.gaps.join(", ") || "None"}</Typography>
                </Stack>
              )}
            </Box>
          )}
