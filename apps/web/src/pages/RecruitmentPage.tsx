...
function PostingForm({ posting, onClose }: { posting: JobPosting | null; onClose: () => void }) {
  ...
  const [form, setForm] = useState({ type: posting?.type ?? "EXTERNAL", title: posting?.title ?? "", description: "", requirements: "", deadline: posting?.deadline?.slice(0, 10) ?? "", isAcademic: posting?.isAcademic ?? false });
  const [error, setError] = useState("");

  const save = useMutation({
    mutationFn: () => posting ? api.put(`/job-postings/${posting.id}`, form) : api.post("/job-postings", { ...form, deadline: new Date(form.deadline).toISOString() }),
  ...
  return (
    <Stack spacing={2} sx={{ mt: 1 }}>
      ...
      <FormControl>
        ...
        <Select value={form.isAcademic ? "academic" : "non-academic"} label="Role Type" onChange={e => setForm(p => ({ ...p, isAcademic: e.target.value === "academic" }))}>
...
function ApplicationsPanel({ posting }: { posting: JobPosting }) {
  ...
  const advance = useMutation({
    mutationFn: (app: Application) => api.put(`/applications/${app.id}/advance`, { publicationEvalScore: posting.isAcademic && app.currentStage === "SCREENING" ? Number(pubScore) : undefined }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["applications", posting.id] }); setAdvanceApp(null); setPubScore(""); },
    onError: (e: any) => setError(e.response?.data?.error?.message ?? "Error"),
  });

  const issueOffer = useMutation({
    mutationFn: (app: Application) => api.post(`/applications/${app.id}/offer`, {}),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["applications", posting.id] }),
    onError: (e: any) => setError(e.response?.data?.error?.message ?? "Error"),
  });
  ...
  return (
    <Box>
      ...
      {isLoading ? <Typography>Loading...</Typography> : (
        <Table size="small">
          <TableHead><TableRow><TableCell>Applicant</TableCell><TableCell>Stage</TableCell><TableCell>Actions</TableCell></TableRow></TableHead>
            ...
            <TableRow key={app.id}>
              ...
              <TableCell>
                <Stack direction="row" spacing={0.5}>
                  {app.currentStage !== "OFFER" && <Button size="small" onClick={() => { setError(""); setAdvanceApp(app); }}>Advance</Button>}
                  {app.currentStage === "SELECTION" && <Button size="small" color="success" onClick={() => issueOffer.mutate(app)}>Issue Offer</Button>}
                </Stack>
              </TableCell>
            </TableRow>
          ))}</TableBody>
      ...
      <Dialog open={!!advanceApp} onClose={() => setAdvanceApp(null)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            {posting.isAcademic && advanceApp?.currentStage === "SCREENING" && (
              <TextField label="Publication Evaluation Score" type="number" value={pubScore} onChange={e => setPubScore(e.target.value)} required />
            )}
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={() => advanceApp && advance.mutate(advanceApp)} disabled={advance.isPending}>Advance</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
...
export default function RecruitmentPage() {
  ...
  return (
    <Box>
      ...
      {canManage && <Button variant="contained" onClick={() => setCreateOpen(true)} sx={{ mb: 2 }}>New Job Posting</Button>}
      {error && <Alert severity="error" sx={{ mb: 2 }}>Failed to load postings</Alert>}
      {isLoading ? <Typography>Loading...</Typography> : (
        <Paper sx={{ mb: 3 }}>
          <Table size="small">
            <TableHead><TableRow><TableCell>Title</TableCell><TableCell>Type</TableCell><TableCell>Deadline</TableCell><TableCell>Academic</TableCell><TableCell>Actions</TableCell></TableRow></TableHead>
            <TableBody>{postings.map(p => (
              <TableRow key={p.id} selected={selectedPosting?.id === p.id}>
                ...
                <TableCell><Chip label={p.isAcademic ? "Yes" : "No"} size="small" color={p.isAcademic ? "primary" : "default"} /></TableCell>
                <TableCell>
                  <Stack direction="row" spacing={0.5}>
