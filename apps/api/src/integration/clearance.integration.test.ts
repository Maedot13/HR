...
function makeBody(id: string, order: number, mode: "SEQUENTIAL" | "PARALLEL") {
  return { id, name: `Body-${id}`, approvalMode: mode, order };
}

function makeTask(id: string, body: ReturnType<typeof makeBody>, status: "PENDING" | "ACTIVE" | "APPROVED") {
...
function makeRecord(tasks: ReturnType<typeof makeTask>[]) {
  return { id: RECORD_ID, employeeId: EMPLOYEE_ID, status: "IN_PROGRESS" as const, initiatedAt: new Date(), completedAt: null, tasks };
}
...
describe("Integration: Full Clearance Pipeline", () => {
  ...
  it("approves sequential task and activates next pending sequential task", async () => {
    ...
    const activationCall = mockPrisma.clearanceTask.update.mock.calls.find((c) => (c[0] as any).where.id === "t2" && (c[0] as any).data.status === "ACTIVE");
  ...
  it("full pipeline: initiate → approve seq → approve par → COMPLETED + INACTIVE", async () => {
    ...
    mockPrisma.clearanceTask.findUnique.mockResolvedValueOnce({ ...parTask, clearanceRecord: makeRecord([{ ...seqTask, status: "APPROVED" } as any, parTask]) });
