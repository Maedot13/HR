...
import type { ActorContext } from "../services/org.service.js";
...
router.put(
  ...
  async (req, res) => {
    const campus = await updateCampus(req.params.id, req.body as Record<string, unknown>, getActor(req));
    res.json({ data: campus });
  }
);
...
router.get(
  ...
  requirePermission(PERMISSIONS.UNIT_READ),
  async (req, res) => {
...
router.delete(
  "/units/:id",
  requirePermission(PERMISSIONS.UNIT_DELETE),
  async (req, res) => {
