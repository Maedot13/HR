...
import type { ActorContext } from "../services/clearance.service.js";
...
router.post("/clearance/bodies", requirePermission(PERMISSIONS.CLEARANCE_BODY_CONFIGURE), async (req, res) => {
  const bodies = req.body as { name: string; approvalMode: "SEQUENTIAL" | "PARALLEL"; order: number }[];
  if (!Array.isArray(bodies) || bodies.length === 0) throw new AppError(422, "VALIDATION_ERROR", "Request body must be a non-empty array");
...
router.put("/clearance/tasks/:id/reject", requirePermission(PERMISSIONS.CLEARANCE_TASK_REJECT), async (req, res) => {
  ...
  if (!rejectionReason) throw new AppError(422, "VALIDATION_ERROR", "rejectionReason is required");
