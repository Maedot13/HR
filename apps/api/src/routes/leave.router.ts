...
import type { ActorContext } from "../services/leave.service.js";
...
router.post("/employees/:id/leave/applications", requirePermission(PERMISSIONS.LEAVE_APPLY), async (req, res) => {
  ...
  const result = await submitApplication(req.params.id, body as Parameters<typeof submitApplication>[1], getActor(req));
