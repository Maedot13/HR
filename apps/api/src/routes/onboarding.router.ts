...
import type { ActorContext } from "../services/onboarding.service.js";
...
router.put(
  "/onboarding/:workflowId/documents/:docId",
  requirePermission(PERMISSIONS.ONBOARDING_UPDATE),
  async (req, res) => {
    res.json({
      data: await markDocumentCollected(
...
router.put(
  "/onboarding/:workflowId/assets/:assetId",
  requirePermission(PERMISSIONS.ONBOARDING_UPDATE),
  async (req, res) => {
    res.json({
      data: await markAssetAssigned(
        req.params.workflowId,
        req.params.assetId,
        getActor(req)
      ),
...
router.post(
  "/onboarding/:workflowId/complete",
  requirePermission(PERMISSIONS.ONBOARDING_COMPLETE),
  async (req, res) => {
