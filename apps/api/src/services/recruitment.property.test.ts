...
function buildApp(id: string, currentStage: string, isAcademic = false, score: number | null = null) {
  return {
    id,
...
describe("Property 9: Recruitment Stage Ordering", () => {
  ...
  it("valid forward transitions succeed for all non-final stages", async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.integer({ min: 0, max: STAGE_ORDER.length - 2 }),
        async (idx) => {
  ...
  it("advancing from OFFER (final stage) throws INVALID_STAGE_TRANSITION", async () => {
    await fc.assert(
      fc.asyncProperty(fc.constant("OFFER"), async (stage) => {
        mockPrisma.application.findUnique.mockResolvedValueOnce(buildApp("app-final", stage));
        let err: unknown;
        try { await advanceStage("app-final", {}, actor); } catch (e) { err = e; }
        expect(err).toBeInstanceOf(AppError);
        expect((err as AppError).code).toBe("INVALID_STAGE_TRANSITION");
      }),
      { numRuns: 10 }
    );
  });
  ...
  it("academic role without publicationEvalScore is rejected at SCREENING with ACADEMIC_SCORE_REQUIRED", async () => {
    await fc.assert(
      fc.asyncProperty(fc.uuid(), async (appId) => {
        mockPrisma.application.findUnique.mockResolvedValueOnce(buildApp(appId, "SCREENING", true, null));
        let err: unknown;
        try { await advanceStage(appId, {}, actor); } catch (e) { err = e; }
        expect(err).toBeInstanceOf(AppError);
        expect((err as AppError).code).toBe("ACADEMIC_SCORE_REQUIRED");
