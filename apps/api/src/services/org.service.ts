...
// ─── Context passed from route handlers ──────────────────────────────────────
...
export async function updateCampus(
  ...
  if ("code" in payload) {
    throw new AppError(
      400,
      "CAMPUS_CODE_IMMUTABLE",
      "Campus code cannot be changed after creation"
    );
  }
...
export async function deleteCampus(id: string, actor: ActorContext) {
  const campus = await prisma.campus.findUnique({ where: { id } });
  if (!campus) throw new AppError(404, "NOT_FOUND", "Campus not found");
  ...
  if (employeeCount > 0) {
    throw new AppError(
      409,
      "CAMPUS_HAS_EMPLOYEES",
      "Cannot delete a campus that has linked employees"
    );
  }
...
export async function listColleges(campusId?: string) {
...
export async function getCollegeById(id: string) {
  ...
  if (!college) throw new AppError(404, "NOT_FOUND", "College not found");
  return college;
}
...
export async function updateCollege(
  ...
  if (!existing) throw new AppError(404, "NOT_FOUND", "College not found");
...
export async function deleteCollege(id: string, actor: ActorContext) {
  const existing = await prisma.college.findUnique({ where: { id } });
  if (!existing) throw new AppError(404, "NOT_FOUND", "College not found");
...
export async function listDepartments(collegeId?: string) {
  ...
  if (actor.role === "ADMIN") {
    ...
    if (!college || college.campusId !== actor.campusId) {
      throw new AppError(
        ...
        "Admin can only manage units within their own campus"
      );
    }
  }
...
export async function listUnits(departmentId?: string) {
...
export async function deleteUnit(id: string, actor: ActorContext) {
  const existing = await prisma.unit.findUnique({ where: { id } });
  if (!existing) throw new AppError(404, "NOT_FOUND", "Unit not found");
 ...
 */
export async function validateUnitBelongsToCampus(
  unitId: string,
  campusId: string
): Promise<void> {
  ...
  if (unit.Department.College.campusId !== campusId) {
    throw new AppError(
      403,
      "CROSS_CAMPUS_VIOLATION",
      "The unit does not belong to the Admin's campus"
    );
  }
}
