...
function makeLeaveType(name: string, overrides: Record<string, unknown> = {}) {
...
describe("Property 10: Leave Balance Never Goes Negative", () => {
  ...
  it("approved duration is deducted exactly from balance", async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.integer({ min: 5, max: 30 }),  // current balance
        ...
        async (balance, _days) => {
          ...
          expect(mockPrisma.leaveBalance.updateMany).toHaveBeenCalledWith(
            expect.objectContaining({ data: expect.objectContaining({ balance: expect.objectContaining({ decrement: expect.any(Number) }) }) })
          );
          const decrementArg = (mockPrisma.leaveBalance.updateMany.mock.calls[0][0] as any).data.balance.decrement;
  ...
  it("rejected application does not change balance", async () => {
    await fc.assert(
      fc.asyncProperty(fc.string({ minLength: 1, maxLength: 50 }), async (reason) => {
        vi.clearAllMocks();
        const app = makeApplication({ status: "PENDING" });
        mockPrisma.leaveApplication.findUnique.mockResolvedValueOnce(app);
  ...
  it("insufficient balance rejects application with currentBalance and shortfall", async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.integer({ min: 0, max: 3 }),   // low balance
        ...
        async (balance, requestedDays) => {
          ...
          if (balance < 5) {
            ...
            const details = appErr.details as { currentBalance: number; shortfall: number };
...
describe("Property 11: Annual Leave Entitlement Calculation", () => {
  ...
  it("entitlement caps at 30 for 11+ years of service", () => {
    fc.assert(
      fc.property(fc.integer({ min: 11, max: 50 }), (years) => {
  ...
  it("academic staff outside July-August are flagged for approval", async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        fc.integer({ min: 0, max: 11 }).filter((m) => m < 6 || m > 7),
        async (month) => {
          vi.clearAllMocks();
          const year = 2026;
          const day = 10;
          ...
          mockPrisma.employee.findUnique.mockResolvedValueOnce(
            makeEmployee({ academicRank: "LECTURER", hireDate: new Date("2020-01-01") })
          );
          mockPrisma.leaveType.findUnique.mockResolvedValueOnce(makeLeaveType("ANNUAL"));
...
describe("Property 12: Research Leave Eligibility Enforcement", () => {
  ...
  it("rejects when rank is below Assistant Professor", async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.constantFrom("LECTURER", null, undefined, ""),
        async (rank) => {
          vi.clearAllMocks();
          mockPrisma.employee.findUnique.mockResolvedValueOnce(
            makeEmployee({ academicRank: rank ?? null, hireDate: new Date("2020-01-01") })
          );
          ...
          expect(err).toBeInstanceOf(AppError);
          expect((err as AppError).code).toBe("LEAVE_ELIGIBILITY_FAILED");
        }
      ),
      { numRuns: 100 }
    );
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (serviceYears) => {
          ...
          mockPrisma.employee.findUnique.mockResolvedValueOnce(
            makeEmployee({ academicRank: "ASSISTANT_PROFESSOR", hireDate })
          );
          mockPrisma.leaveType.findUnique.mockResolvedValueOnce(makeLeaveType("RESEARCH"));
  ...
  it("approves when rank >= Assistant Professor AND service >= 3 years", async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.constantFrom("ASSISTANT_PROFESSOR", "ASSOCIATE_PROFESSOR"),
        ...
        async (rank, serviceYears) => {
          ...
          hireDate.setFullYear(hireDate.getFullYear() - serviceYears);
          mockPrisma.employee.findUnique.mockResolvedValueOnce(
            makeEmployee({ academicRank: rank, hireDate })
          );
...
describe("Property 13: Sabbatical Leave Eligibility Enforcement", () => {
  ...
  it("rejects when not Academic Staff", async () => {
    await fc.assert(
      fc.asyncProperty(fc.constant(null), async (rank) => {
        vi.clearAllMocks();
        mockPrisma.employee.findUnique.mockResolvedValueOnce(
          makeEmployee({ academicRank: rank, hireDate: new Date("2015-01-01") })
        );
  ...
  it("rejects when rank is Lecturer (below Assistant Professor)", async () => {
    await fc.assert(
      fc.asyncProperty(fc.constant("LECTURER"), async (rank) => {
        vi.clearAllMocks();
        mockPrisma.employee.findUnique.mockResolvedValueOnce(
          makeEmployee({ academicRank: rank, hireDate: new Date("2015-01-01") })
        );
  ...
  it("rejects when service < 6 years", async () => {
    await fc.assert(
      fc.asyncProperty(fc.integer({ min: 0, max: 5 }), async (serviceYears) => {
        vi.clearAllMocks();
        const hireDate = new Date();
        hireDate.setFullYear(hireDate.getFullYear() - serviceYears);
        mockPrisma.employee.findUnique.mockResolvedValueOnce(
  ...
  it("approves when all three criteria are met", async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.constantFrom("ASSISTANT_PROFESSOR", "ASSOCIATE_PROFESSOR"),
        fc.integer({ min: 7, max: 20 }), // use 7+ to avoid boundary rounding issues
        async (rank, serviceYears) => {
          vi.clearAllMocks();
          const hireDate = new Date();
          hireDate.setFullYear(hireDate.getFullYear() - serviceYears);
...
describe("Property 14: Sick Leave 8-Month Hard Cap", () => {
  ...
  it("rejects sick leave application when total would exceed 8 months (240 days)", async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.integer({ min: 220, max: 239 }), // already used days close to cap
        ...
        async (usedDays, newDays) => {
          ...
          if (usedDays + newDays > 240) {
            expect(err).toBeInstanceOf(AppError);
            expect((err as AppError).code).toBe("SICK_LEAVE_CAP_EXCEEDED");
          }
        }
      ),
      { numRuns: 100 }
