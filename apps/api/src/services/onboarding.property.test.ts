...
const docArb = (isCollected: boolean) =>
  fc.record({
    id: fc.uuid(),
...
const assetArb = (isAssigned: boolean) =>
  fc.record({
    id: fc.uuid(),
    onboardingWorkflowId: fc.uuid(),
...
function buildWorkflow(
  docs: { id: string; documentType: string; isCollected: boolean; collectedAt: Date | null; onboardingWorkflowId: string }[],
  assets: { id: string; assetName: string; isAssigned: boolean; assignedAt: Date | null; onboardingWorkflowId: string }[]
) {
  return {
    id: "workflow-1",
    applicationId: "app-1",
...
describe("Property 15: Onboarding Completion Requires All Documents and Assets", () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        fc.oneof(
          ...
          fc.tuple(
            fc.array(docArb(true), { minLength: 0, maxLength: 3 }),
            fc.array(docArb(false), { minLength: 1, maxLength: 3 }),
            fc.array(assetArb(true), { minLength: 0, maxLength: 3 })
          ).map(([collectedDocs, uncollectedDocs, assignedAssets]) => ({
          ...
          fc.tuple(
            fc.array(docArb(true), { minLength: 0, maxLength: 3 }),
            fc.array(assetArb(true), { minLength: 0, maxLength: 3 }),
            fc.array(assetArb(false), { minLength: 1, maxLength: 3 })
          ).map(([collectedDocs, assignedAssets, unassignedAssets]) => ({
            docs: collectedDocs,
        ...
        async ({ docs, assets }) => {
          ...
          const details = appErr.details as {
  ...
  it("succeeds and creates employee when all documents collected and all assets assigned", async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.tuple(
          fc.array(docArb(true), { minLength: 0, maxLength: 5 }),
          fc.array(assetArb(true), { minLength: 0, maxLength: 5 })
        ),
        async ([docs, assets]) => {
          vi.clearAllMocks();
          ...
          mockPrisma.$transaction.mockImplementationOnce(async (fn: (tx: typeof mockPrisma) => Promise<unknown>) =>
            fn({ ...mockPrisma, employeeIDCounter: { upsert: vi.fn().mockResolvedValueOnce({ sequence: 1 }) } })
          );
