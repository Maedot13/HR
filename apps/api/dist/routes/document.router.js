...
router.post("/employees/:id/experience-letter", requirePermission(PERMISSIONS.EXPERIENCE_LETTER_GENERATE), async (req, res) => {
    ...
    if (!format || !["PDF", "DOCX"].includes(format))
        throw new AppError(422, "VALIDATION_ERROR", "format must be PDF or DOCX");
    res.status(201).json({ data: await generateExperienceLetter(req.params.id, format, getActor(req)) });
});
