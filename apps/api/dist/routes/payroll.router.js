...
router.post("/payroll/reports", requirePermission(PERMISSIONS.PAYROLL_GENERATE), async (req, res) => {
    ...
    if (!period)
        throw new AppError(422, "VALIDATION_ERROR", "period is required");
...
router.post("/payroll/reports/:id/export", requirePermission(PERMISSIONS.PAYROLL_EXPORT), async (req, res) => {
    ...
    if (!format || !["EXCEL", "PDF", "DOCX"].includes(format))
        throw new AppError(422, "VALIDATION_ERROR", "format must be EXCEL, PDF, or DOCX");
    res.status(201).json({ data: await exportReport(req.params.id, format, getActor(req)) });
});
