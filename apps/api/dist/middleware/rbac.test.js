...
import { computeEffectivePermissions, PERMISSIONS, } from "@hrms/shared";
// ── Helpers ───────────────────────────────────────────────────────────────────
...
function makeReq(role, specialPrivilege) {
    return {
        user: {
            userId: "user-1",
            role,
...
function makeRes() {
    const ctx = { statusCode: null, body: null };
    const res = {
        status(code) {
            ctx.statusCode = code;
            return this;
...
describe("requirePermission middleware", () => {
    it("calls next() when the user has the required permission", () => {
        ...
        const next = vi.fn();
        requirePermission(PERMISSIONS.EMPLOYEE_READ)(req, res, next);
    ...
    it("returns 403 FORBIDDEN when the user lacks the required permission", () => {
        ...
        const { res, ctx } = makeRes();
        const next = vi.fn();
        requirePermission(PERMISSIONS.PAYROLL_GENERATE)(req, res, next);
        expect(next).not.toHaveBeenCalled();
        expect(ctx.statusCode).toBe(403);
        expect(ctx.body.error.code).toBe("FORBIDDEN");
    });
    it("grants access when a special privilege adds the required permission", () => {
        ...
        const { res, ctx } = makeRes();
        const next = vi.fn();
        requirePermission(PERMISSIONS.LEAVE_WITHOUT_PAY_APPROVE)(req, res, next);
        expect(next).toHaveBeenCalledOnce();
        expect(ctx.statusCode).toBeNull();
    });
    ...
    it("does not call next() when 403 is returned", () => {
        ...
        const { res, ctx } = makeRes();
        const next = vi.fn();
        requirePermission(PERMISSIONS.CAMPUS_DELETE)(req, res, next);
        expect(next).not.toHaveBeenCalled();
        expect(ctx.statusCode).toBe(403);
    });
});
...
describe("Property 5: Unauthorized Actions Return HTTP 403", () => {
    ...
    it("returns 403 FORBIDDEN for any user whose effective set lacks the required permission", () => {
        fc.assert(fc.property(arbBaseRole, arbOptionalPrivilege, arbPermission, (role, priv, permission) => {
            const effective = computeEffectivePermissions(role, priv);
            // Only test cases where the user lacks the permission
            if (effective.has(permission))
                return true; // skip — not the scenario under test
            ...
            const hasForbiddenCode = capturedBody?.error?.code === "FORBIDDEN";
    ...
    it("grants access when a special privilege adds the missing permission", () => {
        fc.assert(fc.property(arbBaseRole, arbPrivilege, (role, priv) => {
