...
describe("Property 17: Activity Log Completeness", () => {
    ...
    it("createLog writes an entry with all required fields", async () => {
        await fc.assert(fc.asyncProperty(logParamsArb, async (params) => {
            vi.clearAllMocks();
            ...
            const callData = mockPrisma.activityLog.create.mock.calls[0][0].data;
    ...
    it("queryLogs with campusId filter joins through actingEmployee.campusId", async () => {
        await fc.assert(fc.asyncProperty(fc.uuid(), async (campusId) => {
            vi.clearAllMocks();
            mockPrisma.activityLog.findMany.mockResolvedValueOnce([]);
            await queryLogs({ campusId });
            const callWhere = mockPrisma.activityLog.findMany.mock.calls[0][0].where;
...
describe("Property 18: Activity Log Immutability", () => {
    it("updateLog always throws ACTIVITY_LOG_IMMUTABLE regardless of input", () => {
        fc.assert(fc.property(fc.uuid(), fc.record({ actionType: fc.string() }), (id, data) => {
            let err;
            try {
                updateLog(id, data);
            }
            ...
            expect(err.code).toBe("ACTIVITY_LOG_IMMUTABLE");
    ...
    it("updateLog and deleteLog throw for all roles including SUPER_ADMIN", () => {
        fc.assert(fc.property(fc.uuid(), fc.constantFrom("SUPER_ADMIN", "ADMIN", "HR_OFFICER", "EMPLOYEE"), (id, _role) => {
            let updateErr;
            let deleteErr;
            try {
                updateLog(id, {});
            ...
            expect(updateErr.code).toBe("ACTIVITY_LOG_IMMUTABLE");
            expect(deleteErr.code).toBe("ACTIVITY_LOG_IMMUTABLE");
        }), { numRuns: 100 });
    });
...
//# sourceMappingURL=activityLog.property.test.js.map