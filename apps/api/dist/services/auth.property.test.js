...
describe("Property 8: Password Storage as Bcrypt Hash", () => {
    it("should store passwords as bcrypt hashes with cost factor 12 that verify correctly", async () => {
        await fc.assert(fc.asyncProperty(
        // Generate arbitrary printable ASCII password strings, length 8–72
        fc
            .string({ minLength: 8, maxLength: 72 })
            .filter((s) => s.split("").every((c) => c.charCodeAt(0) >= 32 && c.charCodeAt(0) <= 126)), async (password) => {
