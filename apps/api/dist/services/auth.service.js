 ...
 */
export const revokedRefreshTokens = new Set();
function signAccessToken(payload) {
    const secret = process.env.JWT_ACCESS_SECRET;
    if (!secret)
        throw new AppError(500, "SERVER_MISCONFIGURATION", "JWT_ACCESS_SECRET is not configured");
    return jwt.sign(payload, secret, { expiresIn: ACCESS_TOKEN_TTL });
}
function signRefreshToken(payload) {
    const secret = process.env.JWT_REFRESH_SECRET;
    if (!secret)
        throw new AppError(500, "SERVER_MISCONFIGURATION", "JWT_REFRESH_SECRET is not configured");
 ...
 */
export async function login(employeeId, password, ipAddress) {
 ...
 */
export async function logout(userId, refreshToken, ipAddress) {
 ...
 */
export async function refresh(oldRefreshToken, ipAddress) {
    ...
    let payload;
    try {
        payload = jwt.verify(oldRefreshToken, secret);
    }
    catch (err) {
