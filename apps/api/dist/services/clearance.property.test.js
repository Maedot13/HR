...
function makeBody(id, order, approvalMode = "PARALLEL") {
    return { id, name: `Body-${id}`, approvalMode, order };
}
function makeTask(id, recordId, bodyId, status, body) {
...
function makeRecord(id, employeeId, tasks) {
    return { id, employeeId, status: "IN_PROGRESS", initiatedAt: new Date(), completedAt: null, tasks };
}
describe("Property 16: Clearance Completion Iff All Tasks Approved", () => {
    ...
    it("ClearanceRecord becomes COMPLETED only when every task is APPROVED", async () => {
        await fc.assert(fc.asyncProperty(fc.integer({ min: 2, max: 5 }), async (taskCount) => {
            vi.clearAllMocks();
    ...
    it("Sequential tasks do not become ACTIVE before preceding tasks are APPROVED", async () => {
        await fc.assert(fc.asyncProperty(fc.integer({ min: 3, max: 5 }), async (taskCount) => {
            vi.clearAllMocks();
            ...
            const activationCall = updateCalls.find((call) => call[0].where.id === tasks[1].id && call[0].data.status === "ACTIVE");
            expect(activationCall).toBeDefined();
            for (let i = 2; i < taskCount; i++) {
                const premature = updateCalls.find((call) => call[0].where.id === tasks[i].id && call[0].data.status === "ACTIVE");
                expect(premature).toBeUndefined();
            }
        }), { numRuns: 100 });
    });
    it("Rejecting a task does not complete the clearance record", async () => {
        await fc.assert(fc.asyncProperty(fc.string({ minLength: 1, maxLength: 100 }), async (reason) => {
            vi.clearAllMocks();
...
describe("Property 19: Clearance Triggers Account Deactivation", () => {
    ...
    it("Employee.status is set to INACTIVE when ClearanceRecord transitions to COMPLETED", async () => {
        await fc.assert(fc.asyncProperty(fc.uuid(), async (employeeId) => {
            vi.clearAllMocks();
    ...
    it("approveTask throws INVALID_STATUS for non-ACTIVE tasks", async () => {
        await fc.assert(fc.asyncProperty(fc.constantFrom("PENDING", "APPROVED", "REJECTED"), async (status) => {
            vi.clearAllMocks();
            const body = makeBody("body-0", 0, "PARALLEL");
            const task = makeTask("task-bad", "rec-1", body.id, status, body);
...
//# sourceMappingURL=clearance.property.test.js.map