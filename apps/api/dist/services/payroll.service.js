...
export async function generateReport(period, actor) {
    ...
    try {
        ...
        throw new AppError(500, "REPORT_GENERATION_FAILED", "Unable to generate payroll report. Please try again or contact support.");
    }
}
...
export async function exportReport(reportId, format, actor) {
    // ADDED: Try-catch wrapper for database operations
    try {
        ...
        throw new AppError(500, "EXPORT_FAILED", `Unable to export payroll report in ${format} format. Please try again or contact support.`);
    }
}
export async function validateReport(reportId, actor) {
    const report = await prisma.payrollReport.findUnique({ where: { id: reportId } });
    if (!report) {
