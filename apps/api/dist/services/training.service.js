...
export async function createProgram(data, actor) {
...
export async function assignTraining(employeeId, data, actor) {
    const employee = await prisma.employee.findUnique({ where: { id: employeeId } });
    if (!employee)
        throw new AppError(404, "NOT_FOUND", "Employee not found");
    const program = await prisma.trainingProgram.findUnique({ where: { id: data.trainingProgramId } });
    if (!program)
        throw new AppError(404, "NOT_FOUND", "Training program not found");
...
export async function completeTraining(assignmentId, actor) {
...
export async function getSkillGapReport(employeeId) {
    const employee = await prisma.employee.findUnique({ where: { id: employeeId } });
    if (!employee)
        throw new AppError(404, "NOT_FOUND", "Employee not found");
    const completed = await prisma.trainingAssignment.findMany({ where: { employeeId, status: "COMPLETED" }, include: { TrainingProgram: true } });
    const completedCompetencies = Array.from(new Set(completed.flatMap((a) => a.TrainingProgram.competencies)));
    return { required: [], completed: completedCompetencies, gaps: [] };
}
