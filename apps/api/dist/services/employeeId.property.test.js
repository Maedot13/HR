    ...
    prisma: {
        ...
        campus: {
            findUniqueOrThrow: vi.fn(async ({ where }) => {
                ...
                if (!campus)
                    throw new Error(`Campus not found: ${where.id}`);
                return campus;
            }),
        },
    },
...
describe("Property 6: Employee ID Uniqueness & Format (Requirements 4.1–4.4)", () => {
    it("4.1 – generated IDs match [CampusCode]-[Year]-[NNNNN] format", async () => {
        await fc.assert(fc.asyncProperty(fc.string({ minLength: 2, maxLength: 6 }).map((s) => s.toUpperCase().replace(/[^A-Z0-9]/g, "A")), fc.integer({ min: 2000, max: 2100 }), async (code, year) => {
    ...
    it("4.2 & 4.4 – N sequential IDs for the same campus/year are all distinct", async () => {
        await fc.assert(fc.asyncProperty(fc.integer({ min: 2, max: 20 }), async (n) => {
            counterStore.clear();
    ...
    it("4.2 – sequence increments by exactly 1 for each call", async () => {
        await fc.assert(fc.asyncProperty(fc.integer({ min: 2, max: 15 }), async (n) => {
            counterStore.clear();
            const campusId = "campus-BDU";
            campusStore.set(campusId, { code: "BDU" });
            const year = 2026;
    ...
    it("4.3 – year reset: new year starts sequence at 1, independent of previous year", async () => {
        await fc.assert(fc.asyncProperty(fc.integer({ min: 2000, max: 2099 }), fc.integer({ min: 1, max: 10 }), async (year, prevCount) => {
            counterStore.clear();
            const campusId = "campus-BDU";
            campusStore.set(campusId, { code: "BDU" });
...
//# sourceMappingURL=employeeId.property.test.js.map