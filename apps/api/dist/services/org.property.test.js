...
import { updateCampus, deleteCampus, createDepartment, createUnit, } from "./org.service.js";
// ─── Helpers ──────────────────────────────────────────────────────────────────
const actor = {
    userId: "user-1",
    role: "SUPER_ADMIN",
...
describe("Property 1: Organizational Hierarchy Membership", () => {
    ...
    it("every Department has exactly one College parent and every Unit has exactly one Department parent — no orphaned nodes", async () => {
        await fc.assert(fc.asyncProperty(
        // Generate a random org tree: 1 campus, 1-3 colleges, 1-3 departments per college, 1-3 units per department
        fc.record({
            campusId: fc.uuid(),
            colleges: fc.array(fc.record({
                id: fc.uuid(),
                name: fc.string({ minLength: 1, maxLength: 30 }),
            }), { minLength: 1, maxLength: 3 }),
        }).chain(({ campusId, colleges }) => fc.record({
            campusId: fc.constant(campusId),
            colleges: fc.constant(colleges),
            departments: fc.array(fc.record({
                id: fc.uuid(),
                name: fc.string({ minLength: 1, maxLength: 30 }),
                collegeId: fc.constantFrom(...colleges.map((c) => c.id)),
            }), { minLength: 1, maxLength: 6 }),
        }).chain(({ campusId: cid, colleges: cols, departments }) => fc.record({
    ...
    it("createDepartment associates department with exactly one college via service", async () => {
    ...
    it("createUnit associates unit with exactly one department via service", async () => {
        await fc.assert(fc.asyncProperty(fc.record({
...
describe("Property 2: Campus Code Immutability", () => {
    ...
    it("update payload containing 'code' field always throws CAMPUS_CODE_IMMUTABLE", async () => {
        await fc.assert(fc.asyncProperty(fc.record({
            id: fc.uuid(),
            ...
            await expect(updateCampus(id, payloadWithCode, actor)).rejects.toMatchObject({
                code: "CAMPUS_CODE_IMMUTABLE",
                statusCode: 400,
            });
        }), { numRuns: 100 });
    });
    it("successful update never changes the campus code", async () => {
        await fc.assert(fc.asyncProperty(fc.record({
            id: fc.uuid(),
    ...
    it("update payload with only name field succeeds without touching code", async () => {
        await fc.assert(fc.asyncProperty(fc.record({
            id: fc.uuid(),
            code: fc.string({ minLength: 1, maxLength: 20 }),
            name: fc.string({ minLength: 1, maxLength: 100 }),
...
describe("Property 3: Campus Deletion Blocked When Employees Exist", () => {
    ...
    it("deletion throws CAMPUS_HAS_EMPLOYEES when N >= 1 employees are linked", async () => {
        await fc.assert(fc.asyncProperty(fc.record({
            id: fc.uuid(),
    ...
    it("deletion succeeds when N = 0 employees are linked", async () => {
...
//# sourceMappingURL=org.property.test.js.map