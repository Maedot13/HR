...
export function annualLeaveEntitlement(years) {
...
export async function submitApplication(employeeId, data, actor) {
    ...
    if (!leaveType)
        throw new AppError(404, "NOT_FOUND", "The selected leave type could not be found. Please choose a different type or contact HR.");
    ...
    if (leaveType.name === "RESEARCH") {
        ...
        if (svc < 3)
            unmet.push("requires at least 3 consecutive years of service");
        if (unmet.length > 0) {
    ...
    if (leaveType.name === "SABBATICAL") {
        ...
        const svc = employee.hireDate ? yearsOfService(employee.hireDate) : 0;
        if (svc < 6)
            unmet.push("requires at least 6 continuous years of service");
        if (unmet.length > 0) {
    ...
    if (leaveType.name === "SICK_FULL" || leaveType.name === "SICK_HALF") {
        ...
        const usedDays = existingSick.reduce((sum, app) => {
...
export async function approveApplication(applicationId, actor) {
    const application = await prisma.leaveApplication.findUnique({
        where: { id: applicationId },
        include: { LeaveType: true },
    });
    if (!application)
        throw new AppError(404, "NOT_FOUND", "Leave application not found. It may have been withdrawn or never submitted.");
...
export async function rejectApplication(applicationId, rejectionReason, actor) {
...
export async function listApplications(employeeId) {
