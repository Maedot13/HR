...
function buildCompleteEmployee(id) {
    return {
        id,
...
describe("Property 20: Employee Profile Activation Requires Complete Mandatory Fields", () => {
     ...
     */
    it("activation is rejected with INCOMPLETE_PROFILE when any mandatory field is null, listing exactly the missing fields", async () => {
        await fc.assert(fc.asyncProperty(
        // Generate a non-empty subset of mandatory fields to set to null
        fc
            .subarray(MANDATORY_FIELDS, {
            minLength: 1,
        })
            .map((fields) => fields), async (nullFields) => {
     ...
     */
    it("activation succeeds and sets status to ACTIVE when all mandatory fields are present", async () => {
     ...
     */
    it("missing fields list never includes fields that are present", async () => {
        await fc.assert(fc.asyncProperty(fc
            ...
            const appErr = caughtError;
            const details = appErr.details;
            const presentFields = MANDATORY_FIELDS.filter((f) => !nullFields.includes(f));
...
//# sourceMappingURL=employee.property.test.js.map